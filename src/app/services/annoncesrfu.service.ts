...
@Injectable()
export class AnnonceRfuService extends ApisBaseService {
  redirect_url: string;
  ...
  getAnnonce(params) {
    const url = `${environment.BO_SRV_URL}/bo/ads/${params.id}/`;
    return this.getRequest(url);
  }

  getAnnonceById(id) {
    const url = `${environment.BO_SRV_URL}/bo/ads/${id}/`;
    return this.getRequest(url);
  }

  getAnnonces(params: any = null) {
    console.log('salut ',params);
    let url = new URL(`${environment.BO_SRV_URL}/bo/ads/`);

    if (params) {
      if (typeof params === 'object' && params.hasOwnProperty('uuid')) {
        if (params['uuid']) {
  ...
  searchAnnonces(payload, isFullResults: boolean = false) {
    ...
    if (payload.params['user_id']) {
      url.searchParams.set('uuid', payload.params['user_id']);
    }

    if(payload.categorie && payload.categorie !== 'all'){
      url.searchParams.set('category',payload.categorie)
    }

    if(payload.statut && payload.statut !== 'all'){
      url.searchParams.set('state',payload.statut)
    }
